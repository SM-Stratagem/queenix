...
export const AuthProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const stored = await authClient.getSession();
        if (mounted && stored?.data?.user) {
          const u: any = stored.data.user;
          setSession({
            userId: u.id,
            email: u.email,
  ...
  const signUp = useCallback(async (email: string, password: string, fullName: string) => {
    try {
      const res = await authClient.signUp.email({ email, password, name: fullName, fullName } as any);
      if (res.error) return { ok: false, error: res.error.message };
      const u: any = res.data?.user;
      if (u) {
        setSession({
  ...
  const sendOtp = useCallback(async (phone: string) => {
    try {
      // In production: real OTP provider (Twilio, etc.)
      // For dev: just return ok
      return { ok: true };
    } catch (e: any) {
  ...
  const verifyOtp = useCallback(async (phone: string, otp: string) => {
    try {
      // In production: verify against real provider
